...
app.get('/', (req, res) => {
    res.send('<h1>Server Bot Aktif. Status online.</h1>');
});

app.listen(port, () => {
    console.log(`Server Express untuk monitoring berjalan di port ${port}`);
});
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (qr) {
            console.log("QR Code diterima, silakan scan dari terminal.");
